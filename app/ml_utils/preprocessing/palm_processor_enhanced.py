...
class PalmPreprocessor:
    ...
    def load_and_display_image(self, image_path: str) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if os.getenv("USE_GCS", "false") == "true":
                image_bytes = get_from_gcs(image_path)
    ...
    # def save_image_to_gcs(self, image: np.ndarray, save_path: str) -> None:
    #     """Simpan gambar hasil augmentasi ke Google Cloud Storage"""
    ...
    #     # Upload ke GCS
    #     upload_to_gcs(save_path, image_bytes)
    ...
    def save_image(self, image: np.ndarray, save_path: str) -> None:
        ...
        if os.getenv("USE_GCS", "false") == "true":
            ...
            upload_to_gcs(save_path, content)
        else:
    ...
    def save_augmented_images(
        ...
        # print("dir: ", os.path.join(temp_dir, user_id))

        save_dir = os.path.join(temp_dir, user_id)
        ...
        for idx, (aug_type, image) in enumerate(augmented_dict.items(), 1):
            filename = f"data_{user_id}_{idx}.jpg"
            ...
            if os.getenv("USE_GCS", "false") == "false":
                ...
                upload_to_gcs(file_path, content)

            saved_file_paths.append(file_path)
            # logger.info(f"Saved: {filename} ({aug_type})")

        # logger.info(f"\nSaved {len(augmented_dict)} images in: person_{user_id}")
    ...
    def preprocess_image(self, image_path: Union[str, np.ndarray]) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if landmarks is None:
                return None, "Failed to detect hand landmarks"
            ...
            logger.info(status)
            return self._continue_preprocessing(roi), "Preprocessing successful"
